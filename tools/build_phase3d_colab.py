...
OUTPUT_ROOT = "/content/drive/MyDrive/FedFalsify_Q1/PHASE3D_V13_SCOPE_CONTRAST_FIXTURES"
CHECKOUT_ROOT = "/content/fedfalsify_phase3d_v13"
...
def _one_cell_source() -> str:
    frozen_files = repr(FROZEN_SCIENTIFIC_FILES)
    return f'''# FedFalsify Phase-3D / v13 SCSV-SCC — deterministic zero-new-seed fixture proof
...
REPO_URL = {REPO_URL!r}
PHASE3D_EXECUTION_SOURCE = {PHASE3D_EXECUTION_SOURCE!r}
FROZEN_PHASE3C_SCIENTIFIC_SOURCE = {FROZEN_PHASE3C_SCIENTIFIC_SOURCE!r}
OUTPUT_ROOT = Path({OUTPUT_ROOT!r})
CHECKOUT_ROOT = Path({CHECKOUT_ROOT!r})
FROZEN_SCIENTIFIC_FILES = {frozen_files}
...
def build() -> Path:
    """Generate the deterministic one-cell Colab notebook and matching .py wrapper."""
    source = _one_cell_source()
    ...
    PY_WRAPPER.write_text(source, encoding="utf-8")
    return NOTEBOOK
